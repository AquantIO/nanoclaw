...
export async function routeInbound(event: InboundEvent): Promise<void> {
  ...
  for (const agent of agents) {
    ...
    if (engages && accessOk && scopeOk) {
      ...
      // without requiring another @mention. Uses this wiring's OWN effective
      // thread id — a non-null value already implies the adapter supports
      // threads (resolveThreadPolicy hard-ANDs the capability), so no separate
      // supportsThreads check. DMs, non-threaded platforms, and thread-opted-out
      // wirings skip.
      if (
        !subscribed &&
...
async function deliverToAgent(
  ...
  // Aquant: the adapter itself is needed for the one-time thread-history
  // backfill (fetchThreadHistory), and isTopLevel decides whether a backfill
  // is warranted at all.
  ...
  // threadsEnabled already hard-ANDs adapter.supportsThreads (resolveThreadPolicy),
  // so it supersedes the capability check this line used to carry.
